--2. Show city address collumn without any duplicate
SELECT DISTINCT City_Address FROM student;

--3. Show student name data from computer departement
SELECT Name FROM student WHERE Departement='Computer';

--4. Show nim, name, age and address data sorted from the oldest student
SELECT NIM, Name, City_Address, Age FROM student ORDER BY age DESC;

--5. Show name and age of 3 youngest student from jakarta
SELECT Name, Age FROM student WHERE City_Address='Jakarta'
	ORDER BY age LIMIT 3;

--6. Show name and ipk student from jakarta with ipk under 2,5
SELECT Name, IPK FROM student WHERE City_Address='Jakarta' AND IPK<2.5;

--7. Show any student name from yogyakarta or have age older than 20
SELECT Name, Age FROM student WHERE City_Address='Yogyakarta' OR Age>20;

--8. Show any student name and address not from jakarta and surabaya
SELECT Name, City_Address FROM student WHERE City_Address NOT IN ('Jakarta', 'Surabaya');

--9. Show name, age and ipk student with ipk from 2,5 to 3,5
SELECT Name, Age, IPK FROM student WHERE IPK BETWEEN 2.5 AND 3.5;

--10. Show student name that have letter a in the name
